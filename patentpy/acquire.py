...
def get_bulk_patent_data(year, week, output_file=None):
    ...
    if len(year) != len(week):
        raise ValueError(
            "`year` and `week` parameters should be of equal lengths: \nyear = {}\nweek = {}".format(
                year, week
            )
        )
    # missing/none values error
    if not (all(x is not None for x in year) and all(x is not None for x in week)):
        raise ValueError(
            "`year` and `week` parameters must not have missing or null values: \nyear = {}\nweek = {}".format(
                year, week
            )
        )
    # values of wrong type error
    if not (
        all(isinstance(x, int) for x in year) and all(isinstance(x, int) for x in week)
    ):
        raise TypeError(
            "`year` and `week` parameters should be integer values: \nyear = {}\nweek = {}".format(
                year, week
            )
        )
    # get current day
    ...
    if not all(x >= 1976 and x <= curr_day.year for x in year):
        raise ValueError(
            "`year` value(s) must be between 1976 and current year, inclusive"
        )
...
def convert_to_df(dates_df, output_file=None):
    ...
    if not ("year" == dates_df.columns[0] and "week" == dates_df.columns[1]):
        raise ValueError(
            "`dates_df` parameter must have `year` and `week` columns; current columns = {}".format(
                dates_df.columns
            )
        )
    ...
    if output_file is not None:
        if not isinstance(output_file, str):
            raise ValueError(
                "`output_file` parameter must be a path in the form of a string"
            )
    ...
    for row, curr_year, curr_week in tqdm(
        dates_df.itertuples(), total=dates_df.shape[0]
    ):
        # get file name and url
        curr_file = get_file_name(curr_year, curr_week)
        if curr_file is None:
            print(
                "SKIPPING PATENT DATA FOR WEEK {} OF YEAR {}...".format(
                    curr_week, curr_year
                )
            )
            continue
        ...
        try:
            ...
            print(
                "UNABLE TO GET DATA, SKIPPING PATENT DATA FOR WEEK {} OF YEAR {}...".format(
                    curr_week, curr_year
                )
            )
            continue
        ...
        try:
            if curr_year < 2002:
                ...
                pat_count = xml1_to_df(
                    curr_file, csv_file, True, False
                )  # always append, no header, checked in file
            else:
                pat_count = xml2_to_df(
                    curr_file, csv_file, True, False
                )  # always append, no header, checked in file
            ...
            traceback.print_exception(exc_type, exc_value, e.__traceback__)
            print(
                "UNABLE TO CONVERT ALL PATENT DATA FOR WEEK {} OF YEAR {} DUE TO ABOVE EXCEPTION, SKIPPING REST...".format(
                    curr_week, curr_year
                )
            )
            continue
    ...
    if output_file is None:
        try:
            ...
            if df.shape[0] <= 0:
                ...
                raise Exception(
                    "ERROR, NO PATENTS FOUND, PLEASE RAISE A GITHUB ISSUE @ https://github.com/JYProjs/patentpy/issues"
                )
        except UnicodeDecodeError:
            raise Exception(
                "PANDAS UNABLE TO PARSE FINAL CSV FILE, DATA IS STORED IN STILL STORED IN FILE: {}. DELETE AS NECESSARY!!!".format(
                    temp_output_file
                )
            )
        remove(temp_output_file)
