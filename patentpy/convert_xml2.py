...
def extractFields2(parsed):
    ...
    # get Inventor(s)
    xml_inventors = parsed.findall(
        ".//applicants//applicant//addressbook"
    ) or parsed.findall(".//us-parties//inventors//inventor//addressbook")
    inventors = []
    if xml_inventors:
        for i in range(len(xml_inventors)):
            first_name, last_name = xml_inventors[i].find(
                ".//first-name"
            ), xml_inventors[i].find(".//last-name")
            inventor = "{} {}".format(
    ...
    if xml_assignees:
        for i in range(len(xml_assignees)):
            ...
            if assignee is None:
                first_name, last_name = xml_assignees[i].find(
                    ".//first-name"
                ), xml_assignees[i].find(".//last-name")
                assignee = "{} {}".format(
                    first_name.text if first_name is not None else "",
                    last_name.text if last_name is not None else "",
                )
    ...
    icl_class = parsed.findall(
        ".//classification-ipc//main-classification"
    ) or parsed.findall(".//classification-locarno//main-classification")
    if icl_class:
        ...
        for i in range(len(icl_class)):
            icl_components = [
                icl_class[i].find(".//section"),
                icl_class[i].find(".//class"),
                icl_class[i].find(".//subclass"),
                icl_class[i].find(".//main-group"),
                icl_class[i].find(".//subgroup"),
            ]
            icl_class[i] = "{}{}{} {}{}".format(
                *[j.text if j is not None else "" for j in icl_components]
            )
    ...
    # get Astract
    abstract = parsed.findall("./abstract/p")
...
def xml2_to_df(input_file, output_file, append, header):
    ...
    with open(input_file, "rb") as f1, open(
        output_file, write_mode, encoding="utf-8"
    ) as f2:
        if header:
            f2.write(
