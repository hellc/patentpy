...
def extractFields1(parsed):
    ...
    for i in range(len(xml_inventors)):
        first_name, last_name = xml_inventors[i].find(".//FNM//PDAT"), xml_inventors[
            i
        ].find(".//SNM//PDAT")
        inventor = "{} {}".format(
    ...
    # get Abstract
    abstract = None
    abstract_xml = parsed.xpath("//PARA[@ID = '%s']" % "P-00001")
    if abstract_xml and len(abstract_xml) > 0:
        # print(etree.tostring(abstract_xml, pretty_print=True))
        abstract = abstract_xml[0].findall(".//PTEXT//PDAT")
...
def xml1_to_df(input_file, output_file, append, header):
    ...
    with open(input_file, "rb") as f1, open(
        ...
        parser = etree.XMLParser(
            recover=True
        )  # ignore escaped characters for now, add recognization later
